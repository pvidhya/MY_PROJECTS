...
public class index extends HttpServlet {
    ...
    SaxParser4GameSpeedXMLdataStore p = new SaxParser4GameSpeedXMLdataStore("C://apache-tomcat-7.0.34//webapps//csj//product.xml");

    SaxParser4GameSpeedXMLdataStore l = new SaxParser4GameSpeedXMLdataStore("C://apache-tomcat-7.0.34//webapps//csj//laptop.xml");

    SaxParser4GameSpeedXMLdataStore t = new SaxParser4GameSpeedXMLdataStore("C://apache-tomcat-7.0.34//webapps//csj//tablet.xml");

    SaxParser4GameSpeedXMLdataStore tv = new SaxParser4GameSpeedXMLdataStore("C://apache-tomcat-7.0.34//webapps//csj//TV.xml");

    MySqlDataStoreUtilities m = new MySqlDataStoreUtilities();

    m.deleteProducts();

    // for(Console console: p.consoles)
    // {
    //     String productID = console.id;
    //     String productType = console.type;
    //     int productPrice = console.price;
    //     String productName = console.name;
    //     String productImage = console.image;
    //     try
    //     {
    //     m.insertProducts(productID, productType, productPrice, productName, productImage);
    //     }
    //     catch(Exception e){}
    // }

     for(Console console: l.consoles)
    {
        String productID = console.id;
        String productType = console.type;
        int productPrice = console.price;
...
for(Console console: p.consoles)
    {
      ...
      for(Console console: t.consoles)
    {
        String productID = console.id;
        String productType = console.type;
        int productPrice = console.price;
      ...
      for(Console console: tv.consoles)
    {
        String productID = console.id;
        String productType = console.type;
        int productPrice = console.price;
        ...
        out.println("<script type=\"text/javascript\" src=\"search.js\">");
        out.println("</script>");
...
out.println("</script");
        ...
        out.println("            <li class=\"sign_up\"><a href=\"login.html\">login</a></li>");
        ...
        out.println("<h3>DEALS Matches</h3>");

            File f = new File("C://apache-tomcat-7.0.34//webapps//csj//DealMatches.txt");
            ...
            while(sc.hasNextLine()){
                ...
                String[] deal = line1.split(":");
                String products = deal[0];
                String tweet = deal[1];
                myList.add(products);
                System.out.println(myList);
                //out.println("<h2>"+products+"</h2>");
             for(String product_id1 : myList ) {
       
                Statement stmt = null;

                String query = "SELECT * from ProductList where productName=\""+ product_id1.trim()+"\"";
        ...
        try {
            ...
            while (rs.next()) {
                ...
                out.println("</br>");
                out.println("<tr><td>ProductID</td><td>"+i+ "</td></tr>");
                out.println("<tr><td>ProductName</td><td>"+n+ "</td></tr>");
                out.println("<tr><td>ProductPrice</td><td>"+price+ "</td></tr>");
                out.println("<tr><td>ProductType</td><td>"+type+ "</td></tr>");
                out.println("<tr><td><img src=" + image+ "></td></tr>");
                out.println("</table>"); 
                ...
                out.print("<input type='hidden' name='consoleprice' value="+price+" readonly>");
                ...
                out.print("<input type='hidden' name='price' value= "+price+" readonly>");
                out.print("<input type='hidden' name='type' value="+type+" readonly>");
